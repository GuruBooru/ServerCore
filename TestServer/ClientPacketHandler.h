...
public:
	...
	static void	Init()
	{
		...
		g_packet_handler[PACKET_C_LOGIN] = [](PacketSessionRef& session, BYTE* buffer, int32 len) {
			return HandlePacket < Protocol::C_LOGIN > (Handle_C_LOGIN, session, buffer, len);
		};
		g_packet_handler[PACKET_C_ENTER_GAME] = [](PacketSessionRef& session, BYTE* buffer, int32 len) {
			return HandlePacket < Protocol::C_ENTER_GAME > (Handle_C_ENTER_GAME, session, buffer, len);
		};
		g_packet_handler[PACKET_C_CHAT] = [](PacketSessionRef& session, BYTE* buffer, int32 len) {
	...
	static SendBufferRef	MakeSendBuffer(Protocol::S_ENTER_GAME&packet) { return MakeSendBuffer(packet, PACKET_S_ENTER_GAME); }
	static SendBufferRef	MakeSendBuffer(Protocol::S_CHAT&packet) { return MakeSendBuffer(packet, PACKET_S_CHAT); }
